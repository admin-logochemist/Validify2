...
function SearchTokenData() {
    ...
    return ( <
        ...
        <
        div class = 'next_search' > { showResults ? < Results / > : null } < /div> <
        /div>
        ...
        div className = 'icons_info' > {
            resd.slice(0, 1).map((post, key) => {
                return ( <
                    ...
                    img src = { Ethcoin_icon }
                    /> <
                    li className = 'wrapper_name' > { post.quoteCurrency.symbol }
                    / {post.baseCurrency.symbol}{' '} <
                    /li> <
                    /div>
                );
            })
        ...
        /div> {
            resd.slice(0, 1).map((post, key) => {
                return ( <
                    ...
                    <
                    li className = 'li_div' > { post.baseCurrency.name } <
            ...
            resd.slice(0, 100).map((post, key) => {
                if (post.side == 'BUY') {
                    return ( <
                        ...
                        td className = 'truncate maker_table' > { post.maker.address.slice(0, -2) } <
                        /td> <
                        /tr>
                    );
                } else {
